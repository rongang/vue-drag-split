<template>
    <SplitWindow
        @mouseup="handleDragDropPre"
        @mouseleave="handleMouseLeave"
        :class="layoutClass"
    >
        <slot
            v-if="showPlaceHolder && windowList.length <= 0"
            name="placeHolder"
            class="placeholder"
        >
...
function handleDragStart(event) {
    ...
    dragData.tabEls = [
        ...target.parentNode.querySelectorAll(".header_item[tabkey]"),
    ];
}
...
function handleDragDrop(event) {
    ...
    let elNodes = originSplitViewContentWrapper.querySelectorAll(
        ".split_view_content"
    );
    const dragEl = [...elNodes].find(
        (el) => el.getAttribute("tabViewKey") === dragData.tabKey
    );
    dragData.tabView = dragEl;
    // 目标dom和源dom相同 && 仅有一个标签页
    if (
        dragData.tabEls.length == 1 &&
        dragData.tabView == dragData.respectLayout
    ) {
        return;
    }
    // 目标dom和源dom相同
    if (
        targetDomType == targetDomTypeMap.tab &&
        targetCalc ==
            getParentElByClname(dragData.target, "split_view_label_wrapper")
    ) {
        return;
    }
    // 删除无效dom
...
function addTabEl(tabWrapper, tab) {
    ...
    const appBtnEl = Array.from(tabWrapper?.children ?? []).find((item) =>
        item.className.includes("add_btn_wrapper")
    );
    if (appBtnEl) {
        tabWrapper.insertBefore(tab, appBtnEl);
    } else {
...
function createAndMove(target, dragData) {
    ...
    let dropSplitViewViewWrapper = getParentElByClname(
        target,
        "split_container"
    );
    let paneL = dropSplitViewViewWrapper.querySelector(
        targetElClass + ".split_paneL"
    );
    let paneR = dropSplitViewViewWrapper.querySelector(
        targetElClass + ".split_paneR"
    );
    let paneVnodeSplit;
    ...
    addTabEl(
        paneEl.querySelector(".split_paneR .split_view_label_wrapper"),
        dragData.target
    );
    paneEl
        .querySelector(".split_paneR .split_view_content_wrapper")
        .appendChild(dragData.tabView);
    // 将布局插入到页面中
...
const templateLabel = (win) => {
    return h(
        ...
        [
            renderSlot(Tab, win) || createTab(win) || tabDefault(win),
            templateClose(win),
        ]
    );
};
...
const templateLayout = (win) => {
    const split_view = templateSplitView(
        templateLabel(win),
        templateContent(win)
    );
    const paneVnode = h(
...
function newWindow(win) {
    ...
    if (!document.querySelector(".split_container")) {
        ...
        const splitContentWrapper = getParentElByClname(
            componentIns.vnode.el.querySelector(
                `.header_item[tabkey='${activeTabKey.value}']`
            ),
            "split_content_wrapper"
        );
        addTabEl(
...
function splitWindow(win, type = splitDirectionMap.vertical) {
    ...
    createAndMove(splitContentWrapper, {
        dropPosition:
            type == splitDirectionMap.vertical
                ? dropPositionMap.bottom
                : dropPositionMap.right,
        target: tab,
        tabView: tabView,
    });
    windowList.value.push(win);
...
async function closeWindow(win, event, force = false) {
    ...
    event = event || {
        target: componentIns.vnode.el.querySelector(
            `.header_item[tabkey='${win.key}']`
        ),
    };
    ...
    const tabView = Array.from(
        tab.parentNode.parentNode.nextElementSibling?.children ?? []
    ).find((item) => item.getAttribute("tabviewkey") == tabKey);
...
function closeWindowPost(params, target) {
    ...
    // 删除split_pane
    const splitPaneDirection = splitPane.className.includes(
        splitDirectionMap.vertical
    )
        ? splitDirectionMap.vertical
        : splitDirectionMap.horizontal;
    const splitPanePosition = splitPane.className.includes("split_paneL")
        ? "left"
        : "right";

    requestAnimationFrame(() => {
        if (
            !splitPane?.querySelector(".split_view_content_wrapper")
                ?.firstElementChild
        ) {
...
function clearUselessLayout(elNode) {
    const multiSplitContainer = document.querySelectorAll(
        ".split_container:has(.split_container)"
    );
    for (const elOut of multiSplitContainer) {
        if (elOut.children.length == 1) {
            elOut.parentNode.appendChild(
                elOut.querySelector(".split_container")
            );
            elOut.parentNode.removeChild(elOut);
        }
    }
    const multiSplitPane = document.querySelectorAll(
...
function isEmptySplitContainerEl(splitContainer) {
    ...
    const paneLInnerHtmlIsEmpty = !paneL?.querySelector(
        ".split_view_content_wrapper"
    )?.firstElementChild;
    const paneRInnerHtmlIsEmpty = !paneR?.querySelector(
        ".split_view_content_wrapper"
    )?.firstElementChild;
    if (
        (!paneL && !paneR) ||
